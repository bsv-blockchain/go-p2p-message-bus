...
<table align="center" border="0">
  ...
  <tr>
    ...
    <td align="left">
       ...
       <a href="https://deepwiki.com/bsv-blockchain/go-p2p-message-bus"><img src="https://deepwiki.com/badge.svg" alt="Ask DeepWiki"></a>
    </td>
  </tr>
</table>
...
- Your own peer ID is added automatically, as are the peer IDs of any `StaticPeers` whose ID is known at startup — the entry names it with a `/p2p/` component, or its `/dnsaddr/` resolved. A bare `/dnsaddr/` entry whose DNS lookup fails at startup contributes no ID and is usually warned about, and the set is never rebuilt, so prefer the `/dnsaddr/<host>/p2p/<id>` form when this allowlist is in use. For a relayed (`/p2p-circuit/`) address the ID taken is the target's, not the relay's. `BootstrapPeers` are not added
- Filtered peers never reach the message-receive path, so they are never recorded as topic peers: while filtered they do not appear in `GetPeers()` output at all, even though they stay connected, and no peer-address discovery is attempted for them
